...
async def download_pdf(download_data: DownloadRequest):
    """Generate PDF with results in English"""
    try:
        ...
        # Title
        content.append(Paragraph("Stochastic Differential Equation Solution", title_style))
        ...
        # Solution steps
        content.append(Paragraph("Solution Steps:", styles['Heading2']))
...
def generate_latex_content(data):
    ...
    final_solution_clean = data.final_solution.replace('$', '').replace('\\[', '').replace('\\]', '')
